...
__host__ __device__ inline uint8_t Sbox(uint8_t idx) {
  constexpr uint8_t table[256] = {0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7,
...
__host__ __device__ inline uint8_t Rcon(int idx) {
  constexpr uint8_t table[11] = {0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};
  return table[idx];
}
...
__host__ __device__ inline void KeyExpansion(uint8_t *round_key, const uint8_t *key, const uint8_t *sbox) {
  for (int i = 0; i < kNk; ++i) {
...
#pragma unroll 1
  for (int r = 1; r <= 9; ++r) {
    ...
    uint32_t t0 = te0[s0 >> 24] ^ RotWord8(te0[(s1 >> 16) & 0xff]) ^ RotWord16(te0[(s2 >> 8) & 0xff]) ^
        RotWord24(te0[s3 & 0xff]) ^ rk0;
    uint32_t t1 = te0[s1 >> 24] ^ RotWord8(te0[(s2 >> 16) & 0xff]) ^ RotWord16(te0[(s3 >> 8) & 0xff]) ^
        RotWord24(te0[s0 & 0xff]) ^ rk1;
    uint32_t t2 = te0[s2 >> 24] ^ RotWord8(te0[(s3 >> 16) & 0xff]) ^ RotWord16(te0[(s0 >> 8) & 0xff]) ^
        RotWord24(te0[s1 & 0xff]) ^ rk2;
    uint32_t t3 = te0[s3 >> 24] ^ RotWord8(te0[(s0 >> 16) & 0xff]) ^ RotWord16(te0[(s1 >> 8) & 0xff]) ^
        RotWord24(te0[s2 & 0xff]) ^ rk3;
...
public:
  /**
     ...
     */
  __host__ __device__ Aes128Soft(const uint8_t keys[][16], const uint32_t *te0, const uint8_t *sbox)
      : te0_(te0), sbox_(sbox) {
