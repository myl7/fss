...
int main() {
  ...
  // Init PRG
  uint8_t *keys = (uint8_t *)malloc(2 * kLambda);
  assert(keys != NULL);
  gen_rand_bytes(keys, 2 * kLambda);
  prg_init(keys, 2 * kLambda);
  free(keys);
  ...
  uint8_t *sbufs = (uint8_t *)malloc(kLambda * 3 * thread_num);
  assert(sbufs != NULL);
...
#pragma omp parallel for
  ...
  printf("dpf_eval (us): %lf\n", t_elapsed / (iter_num * domain_size) * 1e6);
  printf("dpf_eval for full domain (ms): %lf\n", t_elapsed / iter_num * 1e3);

  free(sbufs);

  memcpy(y1s, s0s + kLambda, kLambda);
  dpf_eval_full_domain(y1s, 1, k, kAlphaBitlen);
  ...
  for (int i = 0; i < kNumTrials; i++) {
    ...
    uint8_t *y_expected = memcmp(x, alpha, kAlphaBytelen) == 0 ? beta : zero;
