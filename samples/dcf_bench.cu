// DCF CUDA Benchmark: 2^20 parallel Gen/Eval with ChaCha PRG
...
using Prg = fss::prg::ChaCha<4>;  // DCF needs mul=4
using Dcf = fss::Dcf<In, kInBits, Group, Prg>;

// Shared nonce for ChaCha
__constant__ int kNonce[2] = {0x12345678, 0x9abcdef0};

// Generate DCF keys in parallel
__global__ void GenKernel(
    typename Dcf::Cw *cws, const int4 *s0s, const In *alphas, const int4 *betas) {
    ...
    typename Dcf::Cw *key = cws + tid * (kInBits + 1);

    dcf.Gen(key, seeds, alphas[tid], betas[tid]);
}

// Evaluate DCF at given points in parallel
__global__ void EvalKernel(
    int4 *ys, bool b, const int4 *s0s, const typename Dcf::Cw *cws, const In *xs) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= kN) return;
    ...
    const typename Dcf::Cw *key = cws + tid * (kInBits + 1);
    int4 seed = s0s[tid];

    ys[tid] = dcf.Eval(b, seed, key, xs[tid]);
}
...
int main() {
    printf("DCF Benchmark: %d parallel Gen/Eval (input bits: %d)\n", kN, kInBits);
    ...
    size_t cw_size = sizeof(typename Dcf::Cw) * (kInBits + 1) * kN;
    ...
    for (int i = 0; i < kN; i++) {
        ...
        // Test points: mix of less-than-alpha and greater-than-alpha
        h_xs[i] = rand() & ((1 << kInBits) - 1);
    }
    // Ensure first few have known relationship with alpha for verification
    h_xs[0] = (h_alphas[0] > 0) ? h_alphas[0] - 1 : 0;                               // x < alpha
    h_xs[1] = h_alphas[1];                                                           // x == alpha
    h_xs[2] = (h_alphas[2] < ((1 << kInBits) - 1)) ? h_alphas[2] + 1 : h_alphas[2];  // x > alpha

    // Allocate device memory
    typename Dcf::Cw *d_cws;
    ...
    // Verify: y0 XOR y1 = beta if x < alpha (DCF with kLt predicate), else 0
    int correct = 0;
    for (int i = 0; i < kN; i++) {
        int4 sum = {h_y0s[i].x ^ h_y1s[i].x, h_y0s[i].y ^ h_y1s[i].y, h_y0s[i].z ^ h_y1s[i].z,
            h_y0s[i].w ^ h_y1s[i].w};
        bool lt_alpha = (h_xs[i] < h_alphas[i]);
        bool ok;
        if (lt_alpha) {
