...
__global__ void dpf_eval_kernel(bool party, int4 s0,
                                const typename DpfInst::Cw *cws,
...
static torch::Tensor dpf_gen(torch::Tensor s0s, int64_t alpha_lo,
    ...
    auto *cws = reinterpret_cast<typename DpfInst::Cw *>(
...
static torch::Tensor dpf_eval(int64_t party, torch::Tensor s0,
    ...
    if (s0.is_cuda()) {
        ...
        dpf_eval_kernel<<<1, 1>>>(
            b, s0_val,
            reinterpret_cast<const typename DpfInst::Cw *>(
        ...
        TORCH_CHECK(err == cudaSuccess,
                    "dpf_eval_kernel launch failed: ",
                    cudaGetErrorString(err));
        err = cudaDeviceSynchronize();
        TORCH_CHECK(err == cudaSuccess,
                    "dpf_eval_kernel sync failed: ",
                    cudaGetErrorString(err));
    ...
    auto *cws_ptr = reinterpret_cast<const typename DpfInst::Cw *>(
        cws.data_ptr<int32_t>());
...
static torch::Tensor dpf_eval_all(int64_t party, torch::Tensor s0,
                                   torch::Tensor cws) {
    TORCH_CHECK(party == 0 || party == 1, "party must be 0 or 1");
