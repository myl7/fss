...
- x*i = alpha_i (on-path): diff = alpha_i * Delta ^ (1-alpha*i) * Delta = Delta
- x*i != alpha_i (off-path): diff = (1-alpha_i) * Delta ^ (1-alpha*i) * Delta = 0
...
Reason: HCW corrects the non-alpha direction so both parties converge on the high bits. The alpha direction's mismatch is absorbed by the output correction word (ocw/CW\_{n+1}).
